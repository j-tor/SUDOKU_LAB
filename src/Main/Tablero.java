    ...
    public void crearCamposTxt() {
        ...
        for (int i = 0; i < listaTxt.length; i++) {
            for (int j = 0; j < listaTxt[0].length; j++) {
                ...
                txt.setFont(new Font("Arial", Font.BOLD, txtTamañoLetra));
        ...
        for (int i = 0; i < listaTxt.length; i++) {
            for (int j = 0; j < listaTxt[0].length; j++) {
                if (listaTxt[i][j].getText().isEmpty()) {
                    JOptionPane.showMessageDialog(null, "Error. Debe llenar todas las casillas", "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                } else {
